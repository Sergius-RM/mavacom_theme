...
function exclude_custom_post_type_from_search( $query ) {
  ...
  if ( $query->is_search() ) {
      ...
      if ( is_array( $post_types ) ) {
          ...
          $post_types = array_diff( $post_types, array( 'customer_reviews' ) );
      } else {
          // Если $post_types не является массивом, преобразуем его в массив и удаляем "team"
          $post_types = array_diff( explode( ',', $post_types ), array( 'customer_reviews' ) );
      }
...
function my_nav_menu_link_attributes($atts, $item, $args) {
    ...
    if ($item->title == 'Etätuki') {
  
        $atts['id'] = 'etatuki';
        $user_agent = $_SERVER['HTTP_USER_AGENT'];
        $platform = preg_match('/macintosh|mac os x|mac_powerpc/i', $user_agent) ? 'mac' : 'windows';
        $file = $platform == 'mac' ? 'etatuki.dmg' : 'etatuki.exe';
